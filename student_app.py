...
class StudentApp:
    ...
    def create_widgets(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="学生端 - 文件传输系统", font=("Arial", 16, "bold"))
        ...
        self.student_name_label = ttk.Label(info_frame, text="学生姓名：未设置", font=("Arial", 12))
        self.student_name_label.pack(side=tk.LEFT)
        
        change_name_btn = ttk.Button(info_frame, text="修改姓名", command=self.change_student_name)
        change_name_btn.pack(side=tk.RIGHT)
        
        # 连接状态区域
        connection_frame = ttk.LabelFrame(main_frame, text="连接状态", padding="10")
        connection_frame.grid(row=2, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        
        self.connection_status_var = tk.StringVar()
        self.connection_status_var.set("正在搜索教师端...")
        status_label = ttk.Label(connection_frame, textvariable=self.connection_status_var, font=("Arial", 12))
        status_label.pack(side=tk.LEFT)
        
        refresh_connection_btn = ttk.Button(connection_frame, text="重新连接", command=self.auto_discover_teacher)
        refresh_connection_btn.pack(side=tk.RIGHT)
        
        # 老师文件区域
        teacher_frame = ttk.LabelFrame(main_frame, text="老师文件", padding="10")
        teacher_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        teacher_frame.columnconfigure(1, weight=1)
        
        # 刷新按钮
        refresh_btn = ttk.Button(teacher_frame, text="刷新文件", command=self.refresh_teacher_files)
        ...
        # 下载按钮
        download_btn = ttk.Button(teacher_frame, text="下载文件", command=self.download_file)
        download_btn.grid(row=2, column=0, pady=(10, 0))
        
        # 作业上传区域
        work_frame = ttk.LabelFrame(main_frame, text="作业上传", padding="10")
        work_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        work_frame.columnconfigure(1, weight=1)
        
        # 选择文件按钮
        select_file_btn = ttk.Button(work_frame, text="选择作业文件", command=self.select_work_file)
        select_file_btn.grid(row=0, column=0, padx=(0, 10))
        
        # 文件路径显示
        self.file_path_var = tk.StringVar()
        self.file_path_var.set("未选择文件")
        file_path_label = ttk.Label(work_frame, textvariable=self.file_path_var, foreground="gray")
        ...
        # 作业描述
        ttk.Label(work_frame, text="作业描述：").grid(row=1, column=0, sticky=tk.W, pady=(10, 0))
        self.description_var = tk.StringVar()
        description_entry = ttk.Entry(work_frame, textvariable=self.description_var)
        description_entry.grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=(10, 0))
    ...
    def get_student_name(self):
        """获取学生姓名"""
        name = simpledialog.askstring("学生姓名", "请输入您的姓名：", initialvalue=self.student_name)
    ...
    def auto_discover_teacher(self):
        """自动发现教师端"""
        def discover():
            self.connection_status_var.set("正在搜索教师端...")
            ...
            for i in range(1, 255):
                ...
                try:
                    url = f"http://{test_ip}:{self.teacher_port}/api/health"
                    response = requests.get(url, timeout=1)
                    if response.status_code == 200:
                        data = response.json()
                        if data.get("status") == "ok":
                            self.teacher_ip = test_ip
                            self.base_url = f"http://{test_ip}:{self.teacher_port}"
                            self.connection_status_var.set(f"已连接到教师端: {test_ip}")
                            found = True
            ...
            if not found:
                self.connection_status_var.set("未找到教师端，请检查网络连接")
    ...
    def download_file(self):
        ...
        def download():
            try:
                ...
                response = requests.get(f"{self.base_url}/api/teacher/files/{file_id}", timeout=30)
        ...
        def upload():
            try:
                ...
                with open(self.selected_file_path, 'rb') as f:
                    ...
                    response = requests.post(f"{self.base_url}/api/student/work", 
                ...
                if response.status_code == 200:
                    ...
                    if result.get("success"):
                        ...
                        messagebox.showerror("错误", f"上传失败：{result.get('error', '未知错误')}")
                else:
